<?php 
// displaying HEADER datatable 
include "config.php";
$sql="SELECT `id`, `logo_image`, `favicon`, `title`, `email`, `contact`, `address`, `status` FROM `header_settings` WHERE `is_deleted` = 0";
...
{
$roww['id']=$row['id'];
$roww['favicon']=$row['favicon'];
$roww['title']=$row['title'];
$roww['email']=$row['email'];
$roww['contact']=$row['contact'];
$roww['address']=$row['address'];
$roww['image']='<img src="./upload/'.$row['logo_image'].'" height="50px" width="50px">';
